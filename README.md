...
**Khuyến nghị nhanh:**
- Ghi rõ đơn vị trong báo cáo và đồ thị (đã chuyển nhiệt độ thành °C trong script).  
- Bổ sung chú thích trực tiếp lên biểu đồ (ví dụ: highlight năm kỷ lục, ghi chú các site có lượng mưa cao).  
- Phân tích sâu hơn nếu cần: PCA/cluster trên ma trận dị thường, dashboard tương tác (`plotly`/`altair`), hoặc kết hợp dữ liệu Minnesota với dữ liệu năng suất để kiểm tra ảnh hưởng.
