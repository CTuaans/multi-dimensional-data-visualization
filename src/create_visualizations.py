...
def plot_weather_heatmap(df: pd.DataFrame, outdir: str) -> str:
    ...
    """
    monthly_avg = (
        df.groupby(["city", "month"], as_index=False)["avg_temp_c"].mean()
    )
    # Use the precomputed Celsius column `avg_temp_c` so the heatmap
    # is on the same unit scale as the global temperature anomalies.
    
    heatmap_data = monthly_avg.pivot(index="city", columns="month", values="avg_temp_c")
    ...
    plt.xlabel("Month")
    plt.ylabel("City")
    # Colourbar label clarifies units (°C) after conversion from °F.

    outfile = os.path.join(outdir, "weather_heatmap.png")
    plt.tight_layout()
...
def plot_weather_scatter(df: pd.DataFrame, outdir: str) -> str:
    ...
    overlap.
        # Handle precipitation coded as 'T' (trace) by converting to 0
        # and coerce any non-numeric precip entries to numeric 0.0.
    ...
    outdir : str
        Directory to write the output image.
        # Plot temperature (°C) vs humidity; size encodes precipitation (inches)
    ...
    """
    plot_df = df.copy()
    # Treat trace precipitation 'T' and non-numeric values as 0.0
    plot_df["precip"] = plot_df["precip"].replace("T", 0)
    plot_df["precip"] = pd.to_numeric(plot_df["precip"], errors="coerce").fillna(0.0)
        # Y-axis label updated to reflect Celsius units used throughout
...
def plot_minnesota_precip_line(df: pd.DataFrame, outdir: str) -> str:
    ...
    month; values are aggregated by year and month.
        # Ensure precipitation column is numeric so seaborn lineplot can aggregate
    ...
    outdir : str
        Directory to write the output image.
        # Convert precipitation to numeric for accurate plotting
    ...
    """
    plot_df = df.copy()
    plot_df["precip"] = pd.to_numeric(plot_df["precip"], errors="coerce")
...
def main() -> List[str]:
    ...
    # Preprocess weather data: handle precipitation 'T' (trace) and convert temps to °C
    weather_df = weather_df.copy()
    weather_df["precip"] = weather_df["precip"].replace("T", 0)
    weather_df["precip"] = pd.to_numeric(weather_df["precip"], errors="coerce").fillna(0.0)
    # Create Celsius temperature column for consistency with global anomalies
    if "avg_temp" in weather_df.columns:
        weather_df["avg_temp_c"] = (pd.to_numeric(weather_df["avg_temp"], errors="coerce") - 32) * 5.0 / 9.0
    else:
        weather_df["avg_temp_c"] = pd.NA

    # Plot heatmap and scatter (now in °C)
